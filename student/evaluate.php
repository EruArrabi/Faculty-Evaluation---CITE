...
$restriction = $conn->query("
    ...
    FROM 
        restriction_list r 
    INNER JOIN 
        faculty_list f ON f.id = r.faculty_id 
    INNER JOIN 
        subject_list s ON s.id = r.subject_id 
    WHERE 
        r.academic_id = {$_SESSION['academic']['id']} 
        AND r.class_id = {$_SESSION['login_class_id']} 
        AND r.id NOT IN (
            SELECT restriction_id 
            FROM evaluation_list 
            WHERE evaluation_list.academic_id = {$_SESSION['academic']['id']} 
            AND evaluation_list.student_id = {$_SESSION['login_id']}
        )
");
?>
		...
		<div class="col-md-9">
			<div class="card card-outline card-info">
				...
				<div class="card-body">
					...
					<!-- Remarks Section -->
					<div class="mt-3">
						<label for="remarks" class="form-label"><b>Remarks (Comments)Optional:</b></label>
						<textarea id="remarks" name="remarks" class="form-control border border-info" rows="5" placeholder="Enter any additional comments or feedback here..."></textarea><br>
					</div>
